...
def detect_face_for_cropping(image_path):
    """Detect face for cropping purposes using DeepFace with relaxed parameters."""
    try:
        ...
        if result and len(result) > 0 and 'region' in result[0]:
            ...
            # Basic validation - ensure face region isn't too small or too large
            img = Image.open(image_path)
            img_area = img.width * img.height
            face_area = region['w'] * region['h']
            face_ratio = face_area / img_area
            
            # Face should be at least 2% and at most 80% of image area
            if 0.02 <= face_ratio <= 0.8:
                return {
                    'x': region['x'],
                    'y': region['y'], 
                    'w': region['w'],
                ...
                print(f"⚠️ Face region too small/large ({face_ratio:.1%} of image), using center crop")
                return None
        return None
    except Exception as e:
...
def copy_failure_files(source_path, output_path, fail_folder, filename_base):
    ...
    try:
        ...
        # Copy source file with prefix
        fail_source_path = os.path.join(fail_folder, f"SRC_{source_filename}")
        shutil.copy2(source_path, fail_source_path)
        
        # Copy output file with prefix 
        fail_output_path = os.path.join(fail_folder, f"OUT_{output_filename}")
        shutil.copy2(output_path, fail_output_path)
        
        print(f"📁 Copied failure files to 00_FAIL: SRC_{source_filename}, OUT_{output_filename}")
